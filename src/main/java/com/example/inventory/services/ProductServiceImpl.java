...
import com.example.inventory.dao.IProductDao;
	...
	public ResponseEntity<ProductResponseRest> searchById(Long id) {
		...
		try {
			...
			if( product.isPresent()) {
				...
				byte[] imageDescompressed = Utils.decompressZLib(product.get().getPicture());
		...
		try {
			...
			if( listAux.size() > 0) {
				...
				listAux.stream().forEach( (p) -> {
					byte[] imageDescompressed = Utils.decompressZLib(p.getPicture());
